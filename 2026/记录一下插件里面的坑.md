...
考虑之后还是放在 background 呗，数据通过 `zustand/vanilla` 存储，不过比较坑的是 `zustand` 只有订阅没有取消订阅，port 的通信如果对面关闭了还会报错

所以通过 `rsjx` 做一个中转，如果 port 断开了，就取消订阅。

```ts
export const TaskControlStorePortName = "taskControlStorePortName";

// taskControlStore.subscribe 只能订阅不能取消订阅，所以通过rxjs中转一层，避免connect断开之后还在发消息导致报错
const taskControlStoreSubp = new BehaviorSubject(
  taskControlStore.getState().taskQueue
);

taskControlStore.subscribe((state) => {
  taskControlStoreSubp.next(state.taskQueue);
});

chrome.runtime.onConnect.addListener((port) => {
  if (port.name === TaskControlStorePortName) {
    port.postMessage(taskControlStore.getState().taskQueue);

    const sub = taskControlStoreSubp.subscribe((taskQueue) =>
      port.postMessage(taskQueue)
    );
